...
float box_aiou(box a, box b)
{
    boxabs ba = box_c(a, b);
    float w = ba.right - ba.left;
    float h = ba.bot - ba.top;
    float c1 = w*h; //giou
    float c2 = w * w + h * h;  //ziou
    float iou = box_iou(a, b);
    if ((c1 = c2) == 0) { //giou & ziou
        return iou;
    }

    float u1 = box_union(a, b);
    float giou_term = (c1 - u1) / c1; //giou

    float u2 = (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y); //ziou
    float d = u2 / c2;  
    float ar_gt = b.w / b.h;
    ...
    float ziou_term = d + alpha * ar_loss; //ziou
    float aiou_term = (giou_term + ziou_term)/8;                   //aiou
#ifdef DEBUG_PRINTS
    printf("  c: %f, u: %f, riou_term: %f\n", c2, u2, aiou_term);
#endif
    return iou - aiou_term;
}

////////////////////////////////
//// END OF MY SUSTOM LOSS /////
////////////////////////////////
...
//#endif
	...
	//The following codes are calculating the gradient of aiou.
